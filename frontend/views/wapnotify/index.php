...
<div class="mui-content mui-scroll-wrapper" style="padding-top: 0">
 ...
 -->        <ul class="content">
    <?php foreach ($wapnotifyinfo as $key => $value) { ?>
    <li onclick='javascript:window.location.href="/wapnotify/detail?id=<?=$value['id']?>";'>
        <a href="/wapnotify/detail?id=<?= $value['id'] ?>">
            <img src="<?php echo Yii::$app->params['common_url'];?>/img/message.png" alt="" class="">
            <div class="con">
                <p class="title"><span class="ti" style="width: 50%;overflow: hidden;text-overflow: ellipsis;"><?= $value['title'] ?></span> <span><?= $value['createtime'] ?></span></p>
                <p class="mes"><?= mb_substr($value['content'],0,30) ?></p>
            </div>
        </a>
    </li>
    <?php } ?>
