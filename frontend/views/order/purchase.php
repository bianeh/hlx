...
<html>
	...
	<body>
		...
		<!--header end-->
					...
					<div class="cart-list-group ">
						<div id="cartList">
							...
							<div class="list">
								<ul class="mui-table-view up-table-wiew">
									<li class="my-table-view-cell mui-media notarize_table-view-cell">
										<a href="javascript:;">
											<div class="row display_flex">
												<div class="content">
													<img class="mui-media-object mui-pull-left" src="/images/getgoodsmodelpic?Id=<?= $goodsmodelinfo['Id'] ?>">
													<div class="mui-media-body">
														<h1 class="title mui-ellipsis-2"><?= $goodsinfo['name'] ?></h1>
														<p class="mui-ellipsis style"><span><?= $goodsmodelinfo['name'] ?></span></p>
														<div class="price-group">
															<h1 class="price">
																<i class="price-icon">￥</i><?= $goodsmodelinfo['price'] ?>
																<span class="decoration">原价：<?= $goodsinfo['marketprice'] ?></span>
																<span style="color: #ea4a4a">礼盒</span>
																<i class="price-icon">￥</i><?= $boxprice ?>
																<span class="amount" style="float: right">x<?= $num ?></span>
															</h1>
														</div>
													</div>
												</div>
									...
									<li class="order-table-view-cell"><h2 class="order-title"><em>(包含礼盒价)</em> 合计：<i class="price-icon">￥</i><span class="price"><?= $total_price ?>.00</span></h2></li>
                                                                </ul>
							</div>
						</div>
						...
						<div class="list">
							<ul class="mui-table-view up-table-wiew notarize-cost  cell">
								<li class="my-table-view-cell">
									<a>
										<h2 class="h2-title">实付款<span class="cont" id="sum" style="font-size: .4rem"><?= $total_price ?>.00</span></h2>
									</a>
								</li>
								<li class="my-table-view-cell">
									<a>
										<h2 class="h2-title"><span>合计</span><span class="cont" id="dicount"><?= $num ?><i style="color: #000">件商品</i> ￥<?= $total_price ?>.00</span></h2>
									</a>
			...
			<div class="flex_1">
				<h1 class="cont">合计 <i class="price-icon">￥</i><span id="orderSum"><?= $total_price ?>.00</span></h1>
				<span class="fee">含运费</span>
			</div>
			<a class="submit-btn qzzOrderId submit_accountId" id="payment" >结算(<?= $num ?>)</a>
		</div>
        ...
        <script>
            $(document).ready(function(){
                $("#payment").click(function(){
                    var paymoney = <?= $total_price ?>;
                    var goodsid = <?= $goodsinfo['Id'] ?>;
                    var addressid = $("#addressid").val();
                    var num = <?= $num ?>;
                    var Id = <?= $goodsmodelinfo['Id'] ?>;   
                    window.location.href ="/order/payment?paymoney="+paymoney+"&goodsid="+goodsid+"&addressid="+addressid+"&num="+num+"&Id="+Id;
                });
            });
